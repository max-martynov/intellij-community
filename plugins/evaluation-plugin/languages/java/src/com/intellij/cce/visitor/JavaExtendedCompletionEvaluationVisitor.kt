...
class JavaExtendedCompletionEvaluationVisitor : CompletionEvaluationVisitor, JavaRecursiveElementVisitor() {
  ...
  private fun createTokenProperties(reference: PsiJavaCodeReferenceElement): TokenProperties {
    return when (val def = reference.resolve()) {
             ...
             is PsiDeclarationStatement -> declarationProperties()
             else -> null
  ...
  private fun declarationProperties(): TokenProperties {
    return properties(TypeProperty.DECLARATION, SymbolLocation.PROJECT) {}
  }
